...
internal object StatusCodeLookup {
    private val MAP: Map<String, StatusCode>
        ...
        try {
            ...
            repo.connection.use { conn ->
                ...
                val queryString = """
                    ...
                    SELECT ?uri ?label
                    WHERE {
                        <http://id.loc.gov/vocabulary/mstatus>  mads:hasMADSSchemeMember    ?uri .
                ...
                query.evaluate().use { result ->
                    MAP = result.map { StatusCode(it.getValue("uri").stringValue(), it.getValue("label").stringValue()) }.associateBy { TextUtils.getCodeStringFromUrl(it.uri) }
                }
            }
        } catch (e: IOException) {
            throw RuntimeException("Unable to process the status codes file")
        }
    }

    fun lookup(code: String): StatusCode? {
        return MAP[code]
    }

    internal data class StatusCode(val uri: String, val label: String)
}