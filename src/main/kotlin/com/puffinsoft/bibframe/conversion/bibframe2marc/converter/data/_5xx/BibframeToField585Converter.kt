...
internal class BibframeToField585Converter : BibframeToMarcConverter {
    // TODO : no examples
    override fun convert(conn: RepositoryConnection, workData: WorkData, record: BibliographicRecord) {
        query(conn, workData.primaryInstanceId).forEach {
            val builder = DataFieldBuilder().apply { tag = "585" }
    ...
    private fun query(conn: RepositoryConnection, id: Value): List<ExhibitionsNoteData> {
        val exhibitionUri = lookupNoteTypeUri("exhibit")
        ...
        query.evaluate().use { result ->
            return result.map { ExhibitionsNoteData(it.getValue("note"), it.getValue("appliesTo"), if (it.hasBinding("institution")) it.getValue("institution") else it.getValue("institutionId")) }.toList()
        }
    }

    @JvmRecord
    private data class ExhibitionsNoteData(val note: Value, val appliesTo: Value?, val institution: Value?)
}